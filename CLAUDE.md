...
- **Public folder**: In `.gitignore`. Tests MUST NOT rely on files in `public/`
...
- `next/image` is globally mocked in `vitest.setup.ts` to render plain `<img>` tags
- `next/link` is globally mocked in `vitest.setup.ts` to render plain `<a>` tags
